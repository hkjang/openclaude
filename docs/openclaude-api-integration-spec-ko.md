# OpenClaude API 및 외부 연동 명세서

## 1. 문서 목적

이 문서는 OpenClaude가 외부 시스템과 통신하는 지점을 API와 연동 관점에서 분리 정리한 문서다. 설계 전반은 [openclaude-functional-spec-ko.md](D:/project/openclaude/docs/openclaude-functional-spec-ko.md)를 따르며, 본 문서는 요청 목적, 호출 주체, 인증 방식, 실패 처리, 재시도 정책을 중심으로 설명한다.
...
| 진입 명령 | `openclaude remote-control`, `rc`, `bridge`, `sync` |
