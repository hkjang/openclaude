# OpenClaude 릴리즈 / 패키징 / 포크 유지보수 가이드

## 1. 문서 목적

이 문서는 OpenClaude를 배포하거나 포크해서 유지보수할 때 필요한 패키징 구조와 주의점을 소스 기준으로 정리한 운영형 가이드다.

특히 아래 질문에 답하도록 작성했다.

- npm 패키지와 실제 실행 파일은 어떤 경로로 연결되는가
- 왜 실행 파일 이름은 `openclaude`인데 소스 곳곳에는 아직 `claude` 문자열이 남아 있는가
...
1. 현재 패키지명은 `@gitlawb/openclaude`다.
2. 사용자가 실행하는 명령은 `openclaude`다.
3. 엔트리 스크립트는 `bin/openclaude`, 실제 번들 산출물은 `dist/cli.mjs`다.
...
6. UI와 npm 패키징은 OpenClaude로 브랜딩되었지만, native installer, doctor, update, 로컬 설치 wrapper에는 아직 `claude` naming이 많이 남아 있다.
...
```mermaid
flowchart TD
  A["package.json<br/>name=@gitlawb/openclaude<br/>bin=openclaude"] --> B["scripts/build.ts"]
  B --> C["dist/cli.mjs"]
  D["bin/openclaude"] --> C
  E["npm install -g @gitlawb/openclaude"] --> D
  D --> F["사용자 실행 명령<br/>openclaude"]
```
...
| 런처 | `bin/openclaude` | `dist/cli.mjs`가 있으면 실행, 없으면 build 안내 |
...
| 키 | 현재 값 | 의미 |
|---|---|---|
| `name` | `@gitlawb/openclaude` | npm 배포 패키지 이름 |
| `version` | 예: `0.1.7` | 사용자에게 보여줄 OpenClaude 버전 기준 |
| `bin.openclaude` | `./bin/openclaude` | 전역 설치 후 생성되는 실행 명령 |
...
## 5. `bin/openclaude`가 하는 일

`bin/openclaude`는 매우 얇은 런처다.
...
| `MACRO.PACKAGE_URL` | `@gitlawb/openclaude` | npm 업데이트와 설치 안내에 사용 |
...
| update 안내 | `src/cli/update.ts`, `src/utils/autoUpdater.ts`의 `claude update`, `claude install` | 일부 사용자 안내가 아직 `openclaude`가 아님 |
...
- 사용자-facing 브랜딩: OpenClaude
- npm 패키지와 bin 이름: OpenClaude
...
`openclaude --version`은 `src/entrypoints/cli.tsx`에서 `MACRO.DISPLAY_VERSION ?? MACRO.VERSION`을 출력한다.  
...
2. npm 기반 업데이트는 `MACRO.PACKAGE_URL`을 사용하므로 오픈 빌드에서는 `@gitlawb/openclaude`를 본다.
...
- `openclaude --version`이 기대한 버전을 출력하는가
- startup 화면 브랜딩이 Open Claude로 맞는가
- feature-stripped 명령이 의도대로 숨겨지거나 stub 처리되는가

### 10.3 패키징 전

- `npm pack --dry-run` 기준 포함 파일이 기대와 맞는가
- `bin/openclaude`가 `dist/cli.mjs`를 정상 로드하는가
...
4. `claude update`, `claude install` 안내 문구를 `openclaude` 기준으로 정리
...
OpenClaude의 배포 구조는 이미 오픈 브랜딩으로 동작하지만, 설치와 업데이트 계층에는 upstream Claude Code의 흔적이 아직 많이 남아 있다.  
그래서 릴리즈 유지보수는 “패키지 하나 올리기”보다 “브랜딩, installer, doctor, update 경로를 함께 이해하는 일”에 가깝다.

실무적으로는 아래 두 문장을 기억하면 충분하다.

1. 사용자에게 보이는 이름은 `openclaude`지만, 내부 설치/업데이트 일부는 아직 `claude` 기준으로 움직인다.
2. 릴리즈 전에 반드시 `package.json`, `scripts/build.ts`, `bin/openclaude`, `update/doctor/nativeInstaller`를 한 세트로 확인한다.
