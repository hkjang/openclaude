# OpenClaude 한국어 종합 기능 명세서

## 1. 문서 기본 정보

| 항목 | 내용 |
|---|---|
| 문서 목적 | OpenClaude에 포함된 사용자 기능, 운영 기능, 관리자성 설정 기능, 연동 기능, 자동화 기능을 개발·QA·운영·지원이 바로 활용할 수 있는 수준으로 정리한다. |
| 기준 버전 | `@gitlawb/openclaude` `0.1.7` |
...
- 운영 절차 문서: [openclaude-operations-manual-ko.md](D:/project/openclaude/docs/openclaude-operations-manual-ko.md)
- QA 테스트 문서: [openclaude-qa-test-cases-ko.md](D:/project/openclaude/docs/openclaude-qa-test-cases-ko.md)
- API 및 연동 문서: [openclaude-api-integration-spec-ko.md](D:/project/openclaude/docs/openclaude-api-integration-spec-ko.md)

## 2. 시스템 개요

OpenClaude는 터미널 기반 인공지능 코딩 에이전트다. 사용자는 자연어 프롬프트와 슬래시 명령을 통해 코드 설명, 파일 수정, 셸 명령 실행, 외부 도구 호출, 세션 재개, 배경 작업 관리, 원격 세션 연동을 수행할 수 있다. 시스템은 단일 대화형 인터페이스 안에서 다음 계층을 통합한다.
...
| 사용자 유형 | 설명 | 대표 기능 |
|---|---|---|
| 일반 로컬 사용자 | 로컬 프로젝트에서 OpenClaude를 사용하는 기본 사용자 | 대화, 파일 수정, 셸 실행, `/resume`, `/memory`, `/export` |
...
OpenClaude의 실제 접근 제어는 단일 역할 기반이 아니라 아래 축의 조합으로 결정된다.
...
| 제공자 프로필 | `.openclaude-profile.json` | 프로젝트/실행 디렉터리 기준 제공자 프로필 |
...
| 기능군 | 주요 명령/진입점 | 주 대상 |
|---|---|---|
| 기동 및 라우팅 | `openclaude`, `--version`, `daemon`, `remote-control`, `ps/logs/attach/kill`, `--bg` | 사용자, 운영자 |
...
- 진입 경로:
  ...
  - `openclaude daemon`
  - `openclaude ps|logs|attach|kill`
- 상세 동작 절차:
  ...
  4. `.openclaude-profile.json`을 읽어 저장된 제공자 프로필을 시작 환경으로 병합한다.
  5. 제공자 환경 검증을 수행한다.
...
| 저장 프로필 | `.openclaude-profile.json` | 현재 환경과 충돌 시 경고 후 무시 |
...
- 상세 동작 절차:
  ...
  6. 결과를 `.openclaude-profile.json`로 저장한다.
...
| 결과 | 설명 |
|---|---|
| 저장된 프로필 | `.openclaude-profile.json` 생성 또는 갱신 |
...
- 기능 목적: OpenClaude 내부 기능만으로 부족한 시스템 연동을 표준 프로토콜로 확장한다.
...
- 기능 목적: 코드 변경 검토와 협업 피드백 확인을 OpenClaude 내부에서 수행하게 한다.
