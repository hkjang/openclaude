# OpenClaude Request Lifecycle 시퀀스 가이드

## 1. 문서 목적

이 문서는 OpenClaude에서 사용자 입력 1회가 실제로 어떤 단계들을 거쳐 처리되는지 코드 기준으로 설명하는 실행 흐름 문서다.  
...
- [메모리 / 컨텍스트 압축 상세 설계](D:/project/openclaude/docs/openclaude-memory-context-compaction-ko.md)
- [API 가이드](D:/project/openclaude/docs/openclaude-api-guide-ko.md)
- [권한 / 보안 매트릭스](D:/project/openclaude/docs/openclaude-permission-security-matrix-ko.md)
...
1. OpenClaude의 요청 처리 핵심은 `src/query.ts` 의 `query()` / `queryLoop()` 다.
...
자세한 메모리 구조는 [메모리 / 컨텍스트 압축 상세 설계](D:/project/openclaude/docs/openclaude-memory-context-compaction-ko.md)를 함께 보면 된다.
