...
OpenClaude의 `src`는 단순 MVC 구조가 아니라 아래 다층 구조에 가깝습니다.
...
- 외부 프로세스나 다른 앱이 OpenClaude를 포함할 때 이 계층을 먼저 보게 됩니다.
...
OpenClaude의 `src`는 다음 네 층으로 이해하면 가장 빠릅니다.
