# OpenClaude 명령어 실전 가이드

## 1. 문서 목적

이 문서는 OpenClaude의 명령 체계를 소스 기준으로 풀어서, 어떤 명령이 어디에서 등록되고, 어떤 상황에서 어떤 진입점을 선택해야 하는지 빠르게 이해할 수 있게 만든 운영 가이드입니다.
...
- 이 저장소 패키지의 실제 실행 예시는 `openclaude` 기준으로 읽는 것이 안전합니다.

## 2. 명령 표면 구조

OpenClaude의 명령 표면은 크게 네 층으로 나뉩니다.

| 구분 | 진입 방식 | 주 용도 |
|---|---|---|
| 시작 옵션 | `openclaude --flag ...` | 실행 모드, 권한, 모델, 세션 제어 |
| CLI 서브커맨드 | `openclaude auth ...`, `openclaude plugin ...` | 관리형 작업, 설치, 상태 조회 |
...
| 명령 | 목적 |
|---|---|
| `openclaude auth login` | Anthropic 계정 로그인 |
| `openclaude auth status` | 현재 인증 상태 확인 |
| `openclaude auth logout` | 인증 종료 |
| `openclaude plugin validate <path>` | 플러그인/마켓플레이스 manifest 검증 |
| `openclaude plugin install <plugin>` | 플러그인 설치 |
| `openclaude plugin update <plugin>` | 플러그인 업데이트 |
| `openclaude plugin disable [plugin]` | 플러그인 비활성화 |
| `openclaude mcp serve` | MCP 서버 기동 |
| `openclaude mcp add-json <name> <json>` | MCP 서버 정의 추가 |
| `openclaude mcp get <name>` | MCP 서버 상태 확인 |
| `openclaude doctor` | 환경 진단 |
| `openclaude agents` | 설정된 에이전트 목록 조회 |
| `openclaude setup-token` | 장기 토큰 설정 |

### 6.2 feature gate 또는 특수 모드 기반 서브커맨드

| 명령 | 조건 | 설명 |
|---|---|---|
| `openclaude server` | `DIRECT_CONNECT` | direct connect 세션 서버 시작 |
| `openclaude open <cc-url>` | `DIRECT_CONNECT` | `cc://` 또는 `cc+unix://`로 서버 연결 |
| `openclaude ssh <host> [dir]` | `SSH_REMOTE` | 원격 호스트에서 CLI 실행 |
| `openclaude remote-control` | `BRIDGE_MODE` | remote control 연결 |
| `openclaude assistant [sessionId]` | `KAIROS` | assistant 모드 |
| `openclaude auto-mode` | feature gate | auto mode 관련 흐름 |
...
```powershell
openclaude -p "현재 저장소의 변경점을 요약해줘" --output-format text
```
...
```powershell
openclaude --setting-sources project,local
```
...
```powershell
openclaude plugin validate D:\project\sample-plugin
```
...
- [openclaude-feature-flag-build-guide-ko.md](D:/project/openclaude/docs/openclaude-feature-flag-build-guide-ko.md)
- [openclaude-plugin-hook-guide-ko.md](D:/project/openclaude/docs/openclaude-plugin-hook-guide-ko.md)
- [openclaude-server-mode-guide-ko.md](D:/project/openclaude/docs/openclaude-server-mode-guide-ko.md)
- [openclaude-troubleshooting-guide-ko.md](D:/project/openclaude/docs/openclaude-troubleshooting-guide-ko.md)
