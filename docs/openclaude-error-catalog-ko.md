# OpenClaude 오류 메시지 카탈로그

## 1. 문서 목적

이 문서는 OpenClaude에서 실제로 사용자 또는 운영자가 마주치는 대표 오류 메시지를 코드 기준으로 분류한 카탈로그다.  
...
- [서버 모드 가이드](D:/project/openclaude/docs/openclaude-server-mode-guide-ko.md)
...
plugin 구조 자체는 [플러그인 / 훅 가이드](D:/project/openclaude/docs/openclaude-plugin-hook-guide-ko.md)에서 더 자세히 다룬다.
