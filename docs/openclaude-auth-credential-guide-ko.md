# OpenClaude 인증 / 자격증명 우선순위 가이드

## 1. 문서 목적

이 문서는 OpenClaude가 어떤 인증 정보를 어떤 우선순위로 읽는지, 왜 사용자가 기대한 자격증명과 다른 값이 선택될 수 있는지, provider별로 어디를 확인해야 하는지를 코드 기준으로 풀어쓴 운영 가이드다.
...
OpenClaude의 인증은 하나의 축으로 끝나지 않는다.
...
즉, OpenClaude는 단순히 인증을 읽고 끝내는 것이 아니라 “지금 왜 이런 인증 조합이 위험한지”를 UI에서도 설명하려고 한다.
...
- provider 호출 구조는 `openclaude-api-guide-ko.md`
- build variant 차이는 `openclaude-feature-flag-build-guide-ko.md`
- Remote Control 쪽 인증 요구사항은 `openclaude-remote-control-bridge-guide-ko.md`
- 증상별 대응은 `openclaude-troubleshooting-guide-ko.md`

---

## 16. 한 줄 요약

OpenClaude의 인증은 “어떤 provider를 쓰는가”와 “그 provider 안에서 어떤 소스가 우선권을 가지는가”의 두 단계로 결정되며, `apiKeyHelper`, inference-only OAuth, `auth.json`, secure storage가 각각 별도 우선순위와 제약을 가진다.
