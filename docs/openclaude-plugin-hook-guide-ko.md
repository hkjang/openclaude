# OpenClaude 플러그인 및 훅 추가 가이드

## 1. 문서 목적

이 문서는 OpenClaude 소스를 기준으로 플러그인과 훅이 실제로 어떻게 로드되고 실행되는지 설명하고, 새 기능을 추가하려는 개발자가 바로 따라 할 수 있는 수준의 실전 가이드를 제공한다.

대상 독자는 다음과 같다.

- OpenClaude에 새 확장 기능을 넣고 싶은 개발자
...
| 구분 | 한 줄 설명 | 대표 소스 |
|---|---|---|
| 플러그인 | OpenClaude에 새 명령, 에이전트, 훅, MCP, LSP, 출력 스타일을 묶어서 추가하는 패키지 | `src/utils/plugins/pluginLoader.ts` |
...
1. 플러그인 디렉터리 작성
2. `openclaude plugin validate <플러그인 경로 또는 plugin.json 경로>` 실행
...
- 이 저장소의 패키지 실행 파일은 `openclaude`다. 로컬 설치 환경에서 문서 예시를 그대로 따라 할 때는 `openclaude plugin validate`를 기준으로 보면 된다.
...
## 20. 플러그인을 OpenClaude 코드에 내장하고 싶을 때
...
2. 상대 경로가 `./`로 시작하는지 확인
3. `openclaude plugin validate`로 schema 문제를 먼저 제거
