# OpenClaude QA 테스트 케이스

## 1. 문서 목적

이 문서는 OpenClaude의 핵심 기능을 검증하기 위한 QA 테스트 케이스 모음이다. 상세 기능 정의는 [openclaude-functional-spec-ko.md](D:/project/openclaude/docs/openclaude-functional-spec-ko.md)를 따른다.
...
|---|---|---|---|---|
| OC-QA-001 | OpenAI 환경으로 정상 기동 | `CLAUDE_CODE_USE_OPENAI=1`, 유효한 `OPENAI_API_KEY` | 1. `openclaude` 실행 | 시작 화면 후 대화 입력 가능 |
| OC-QA-002 | 로컬 base URL에서 API key 없이 기동 | Ollama 또는 로컬 OpenAI 호환 서버 실행 중 | 1. `OPENAI_BASE_URL`을 로컬 주소로 설정 2. `openclaude` 실행 | API key 오류 없이 시작 |
| OC-QA-003 | placeholder API key 차단 | `OPENAI_API_KEY=SUA_CHAVE` | 1. `openclaude` 실행 | placeholder 경고 후 종료 |
| OC-QA-004 | Codex auth.json 자동 인식 | 유효한 `~/.codex/auth.json` 존재 | 1. Codex provider 설정 2. 기동 | Codex provider로 시작 |
| OC-QA-005 | Codex account id 누락 처리 | auth 파일에서 account id 제거 | 1. `openclaude` 실행 | account id 누락 오류 출력 |
...
| OC-QA-027 | 정책에 의한 원격 제어 차단 | 정책에서 `allow_remote_control=false` | 1. `openclaude remote-control` | 차단 메시지 출력 |
...
| OC-QA-035 | BG 세션 목록 조회 | `--bg` 실행 세션 존재 | 1. `openclaude ps` 실행 | 세션 목록 표시 |
...
- 기능 기준 문서: [openclaude-functional-spec-ko.md](D:/project/openclaude/docs/openclaude-functional-spec-ko.md)
- 운영 절차 문서: [openclaude-operations-manual-ko.md](D:/project/openclaude/docs/openclaude-operations-manual-ko.md)
- API/연동 문서: [openclaude-api-integration-spec-ko.md](D:/project/openclaude/docs/openclaude-api-integration-spec-ko.md)
