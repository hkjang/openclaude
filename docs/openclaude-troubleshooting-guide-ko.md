# OpenClaude 트러블슈팅 가이드

## 1. 문서 목적

이 문서는 OpenClaude를 운영하거나 개발할 때 자주 만나는 문제를 증상 기준으로 빠르게 좁혀가기 위한 실전 대응 가이드다. 단순 FAQ가 아니라 “어떤 소스 파일을 보면 원인을 확정할 수 있는가”까지 함께 정리했다.
...
- `openclaude server`가 필요한지 다시 판단해야 할 수 있다
...
- build/feature 차이는 `openclaude-feature-flag-build-guide-ko.md`
- Remote Control 구조는 `openclaude-remote-control-bridge-guide-ko.md`
- 인증 우선순위는 `openclaude-auth-credential-guide-ko.md`
- API 흐름은 `openclaude-api-guide-ko.md`
- direct connect는 `openclaude-server-mode-guide-ko.md`

---

## 7. 한 줄 요약

OpenClaude 장애 대응은 “기능이 빌드에 포함됐는가 → 올바른 provider/auth가 선택됐는가 → 네트워크/재시도 계층에서 막히는가 → remote-control과 server 중 어느 시스템 문제인가” 순서로 좁혀가면 가장 빠르다.
