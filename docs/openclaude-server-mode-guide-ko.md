# OpenClaude 서버 기동 가이드

## 1. 이 문서가 답하는 질문

이 문서는 다음 질문에 답하기 위해 작성했다.

- OpenClaude를 서버처럼 띄우는 방법이 실제로 있는가
- 있다면 어떤 명령으로 실행하는가
...
실제 등록 코드는 [main.tsx](D:/project/openclaude/src/main.tsx:3944) 부근에 있다.
...
### 2.2 설치 바이너리 이름은 `openclaude`

소스 내부 도움말과 `program.name()` 쪽에는 상속된 `claude` 문자열이 많이 남아 있지만, 이 저장소의 실제 패키지 바이너리는 `openclaude`다. 따라서 사용자는 아래처럼 실행하면 된다.
...
- [package.json](D:/project/openclaude/package.json:6)
- [main.tsx](D:/project/openclaude/src/main.tsx:3944)

---

## 3. 어떤 종류의 서버인가

이 서버는 “HTTP + WebSocket 기반으로 OpenClaude 세션을 외부 클라이언트가 붙을 수 있게 만드는 direct connect session server”에 가깝다.
...
- 세션 생성 요청: [createDirectConnectSession.ts](D:/project/openclaude/src/server/createDirectConnectSession.ts)
- 응답 schema: [types.ts](D:/project/openclaude/src/server/types.ts)
- WS 연결 관리자: [directConnectManager.ts](D:/project/openclaude/src/server/directConnectManager.ts)
...
| 구분 | `openclaude server` | `openclaude remote-control` |
|---|---|---|
...
`remote-control`은 [bridgeMain.ts](D:/project/openclaude/src/bridge/bridgeMain.ts:1908)에서 스스로를 “persistent server that accepts multiple concurrent sessions”라고 설명하지만, 그 의미는 direct connect용 HTTP/WS 세션 서버라기보다 Claude 웹 서비스와 연결된 브리지 런타임에 가깝다.
...
```bash
openclaude server --host 127.0.0.1 --port 8080
```
...
- [main.tsx](D:/project/openclaude/src/main.tsx:3945)

### 5.3 고정 토큰을 지정해서 실행

```bash
openclaude server --host 127.0.0.1 --port 8080 --auth-token "my-strong-token"
```
...
- [main.tsx](D:/project/openclaude/src/main.tsx:3982)

### 5.4 기본 작업 디렉터리 지정

```bash
openclaude server --host 127.0.0.1 --port 8080 --workspace "D:\\project\\openclaude"
```
...
- [types.ts](D:/project/openclaude/src/server/types.ts:22)
- [main.tsx](D:/project/openclaude/src/main.tsx:3945)
...
```bash
openclaude server --unix /tmp/openclaude.sock
```
...
```bash
openclaude server --host 127.0.0.1 --port 8080 --idle-timeout 0 --max-sessions 4
```
...
- [createDirectConnectSession.ts](D:/project/openclaude/src/server/createDirectConnectSession.ts:49)
- [types.ts](D:/project/openclaude/src/server/types.ts:5)
...
- [directConnectManager.ts](D:/project/openclaude/src/server/directConnectManager.ts:43)
...
- `cc://` 감지: [main.tsx](D:/project/openclaude/src/main.tsx:610)
- 내부 `open` 명령: [main.tsx](D:/project/openclaude/src/main.tsx:4042)
...
1. `openclaude server`를 실행해 콘솔 배너를 확인하고
...
```bash
openclaude server --host 127.0.0.1
```
...
- [main.tsx](D:/project/openclaude/src/main.tsx:3964)
...
```bash
openclaude server --host 127.0.0.1 --port 8080 --auth-token "long-random-token" --max-sessions 8
```
...
| 증상 | 먼저 볼 것 |
|---|---|
| `openclaude server` 명령이 없음 | 현재 빌드에 `DIRECT_CONNECT`가 포함됐는지 확인 |
...
1. [main.tsx](D:/project/openclaude/src/main.tsx:3944)
2. [createDirectConnectSession.ts](D:/project/openclaude/src/server/createDirectConnectSession.ts)
3. [types.ts](D:/project/openclaude/src/server/types.ts)
4. [directConnectManager.ts](D:/project/openclaude/src/server/directConnectManager.ts)
5. [bridgeMain.ts](D:/project/openclaude/src/bridge/bridgeMain.ts:1908)

---

## 13. 한 줄 정리

OpenClaude는 소스상 direct connect용 `server` 모드를 가지고 있고 `openclaude server`로 기동하는 설계가 맞다. 다만 이 기능은 `DIRECT_CONNECT` feature gate 뒤에 있고, 현재 체크아웃에는 서버 내부 구현 파일 일부가 빠져 있어 “존재와 연결 프로토콜은 확인 가능하지만 배너/라우터 세부 구현은 이 스냅샷만으로 완전 검증 불가” 상태라고 보는 것이 가장 정확하다.
