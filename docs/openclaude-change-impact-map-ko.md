...
이 문서는 OpenClaude 소스를 수정할 때 어떤 파일과 문서, 테스트 범위를 함께 봐야 하는지 빠르게 판단하기 위한 유지보수 가이드다.
...
1. OpenClaude는 CLI 하나처럼 보여도 실제로는 `entrypoint -> bootstrap -> query loop -> tools -> persistence -> settings/policy`가 강하게 연결된 구조다.
...
| 빌드 / 패키징 | `package.json`, `scripts/build.ts`, `bin/openclaude`, `src/cli/update.ts` | 배포 결과물, 버전 표기, 업데이트, 내부 기능 제거가 함께 바뀜 |
...
| 변경 포인트 | 같이 확인할 코드 | 같이 갱신할 문서 |
|---|---|---|
| 패키지명 / bin | `package.json`, `bin/openclaude`, README | 패키징/포크 가이드 |
...
| `scripts/build.ts`, `package.json`, `bin/openclaude` | Feature Flag / 빌드 가이드, 릴리즈/패키징/포크 가이드 |
...
OpenClaude에서 고위험 변경은 대부분 “파일 하나”가 아니라 “흐름 하나”를 건드린다.  
